...
@ルート("POST", "/api/team/standard-data", 権限="コーチ")
def 標準データ追加(リクエスト_: リクエスト):
    """アプリの更新で増えた標準のスキル・ドリル・作戦テンプレートのうち、チームにないものだけを追加する。"""
    return 初期データ.標準データを追加(リクエスト_.db, リクエスト_.チームID, リクエスト_.ユーザー["id"])
